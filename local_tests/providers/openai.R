...
llt_test("structured output returns valid data", {
  ...
  result <- llm_message("Extract: Jane Doe is 28 years old.") |>
    chat(openai(), .json_schema = schema)
...
llt_test("embed returns tibble with embeddings", {
  result <- c("Hello world", "Goodbye world") |> embed(openai())
...
llt_test("multi-turn tool use completes", {
  ...
  ) |> chat(openai(), .tools = list(city_tool, capital_tool), .max_tool_rounds = 5)
  llt_expect_reply(result)
})
...
llt_test("streamed tool use assembles and completes", {
  ...
  result <- llm_message("What is the temperature in Berlin and in Reykjavik? Use the tool for both.") |>
    chat(openai(), .tools = temp_tool, .stream = TRUE)
