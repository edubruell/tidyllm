...
To stream into a user interface rather than the console, use `send_chat()`, which dispatches a request without blocking the session and reports each delta through `.on_chunk`. See the article on [using tidyllm in Shiny](articles/tidyllm_shiny.html), and the example app it walks through: `tidyllm_example_app("model_explainer")`.
