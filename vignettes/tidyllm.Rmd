...
The OpenAI and the Google Gemini API work with a schema provided by `tidyllm_schema()`. Other API providers like `ollama()`, `groq()`, and `mistral()` also support structured outputs through a simpler JSON mode, accessible with the `.json` argument in these provider functions. Since these APIs do not currently support native schema enforcement, you’ll need to prompt the model to follow a specified format directly in your messages. Although `get_reply_data()` can help extract structured data from these responses when you set `.json=TRUE` in each of the API provider functions, the model may not always adhere strictly to the specified structure. Once these APIs support native schema enforcement, **tidyllm** will integrate full schema functionality for them.
...
```{r, eval = FALSE}
address <- llm_message("Imagine an address in JSON format that matches the schema.") |>
        chat(ollama(),.json_schema = address_schema)
```
