...
```
This output demonstrates that the model accurately interprets the prompts, assigning each occupation description to a suitable category. With this confirmation, we can confidently leverage tidyllm’s built-in batch-processing functions to classify the entire set of messages in `classification_tasks` using Anthropic's Claude-3.5-Sonnet model.
...
To initiate a batch request, we use the `send_claude_batch()` function. This function returns the same list of classification tasks that we sent, but marked with an attribute that contains a batch-id from the Claude API as well as unique names for each list element that can be used to stitch together messages with replies.  If you supply a named list of messages, these names are used as identifiers (ensuring they are unique before submission). For convenient access later, we save the batch output locally, allowing us to resume processing from the batch even after closing the R session:
```{r batch_send, eval=FALSE}
classification_tasks |>
  send_claude_batch(.temperature = 0) |>
  write_rds(here::here("sub_sample_batch.rds"))
```

We can check the status of a batch with:
```{r check_batch, eval=FALSE}
read_rds(here::here("local_wip","sub_sample_batch.rds")) |>
  check_claude_batch()
```
...
```
We now see that our batch was created and its status is `ended`. All 442 requests succeeded. Batch results can be downloaded within  29 days after creation. You can see all available batches in the API with `list_claude_batches()`. To download the results of our batch we use the `fetch_claude_batch()` function:

```{r fetch_batch, eval=FALSE}
occ2_codes <- read_rds(here::here("local_wip","sub_sample_batch.rds")) |>
  fetch_claude_batch() |>
  map_chr(get_reply) |>
  parse_number()
...
To test different prompts and models systematically we need to create a flexible classifier function that can handle different prompts or models that can work with any api-function including local models that do not support batch requests.  This function can take the open occupation, our ground-truth for `occ2` a `prompt` and `prompt_id`, a `api_function` and a `model` as arguements allowing us to test each occupation classification across a grid of prompts and models.
...
In our specific case we already  had a clear favorite final choice without even needing to do further testing, because we already know test performance for Claude Sonnet 3.5, since we manually checked it.  So no further check of prompt or models on the test data is needed here. We can just run our entire sample through `claude_send_batch()`
