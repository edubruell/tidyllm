...
```
> ⚠️ **Note:** This article discusses code that uses features from the development version 0.1.11 of **tidyllm** that are not in the last CRAN release. You can install the development version with:
...
To test different prompts and models systematically we need to create a flexible classifier function that can handle different prompts or models that can work with any api-function including local models that do not support batch requests.  This function can take the open `occupation` question, our ground-truth for `occ2` a `prompt` and `prompt_id`, a `api_function` and a `model` as arguements allowing us to test each occupation classification across a grid of prompts and models.
