...
2. **Models:**
  ...
  - **Gemma2:9B:** Another candidate model, which performs well for classification tasks, but is double the size of Lama3.2 and is therefore somewhat slower.
...
chatgpt_grid %>%
  ...
  accuracy(truth = occ2_truth, estimate = occ2_predict)
```
Still worse accuracy than Claude Sonnet and not much better than **gemma2**. So it looks like the easiest solution for our task is to spend roughly 2,50$ for the Anthropic API and use Claude Sonnet. We already know test performance for this model on the original split, since we manually checked it. So the final step is to use this model on all data.
