...
First, we prepare a list of messages for all PDFs in the folder by applying `llm_message()` with a prompt to the first five pages of each document. Even though `gpt-5.6-terra` can process up to 128,000 tokens (roughly 80-90 pages of English text), we limit the input to five pages for demonstration purposes; the introduction is usually enough to get a first overview of a paper.
...
At current `gpt-5.6-terra` batch pricing the per-document cost is a few cents; processing a folder of 50 papers costs well under a dollar.
