...
```
> ⚠️ **Note:**  This article refers to the development version 0.2.6. of **tidyllm**, which has a chnaged interface compared to the last CRAN release. You can install the current development version directly from GitHub using devtools:
...
tibble(
  ...
  timestamp = as.POSIXct("2024-11-13 07:41:29"),
  prompt_tokens = 4356L,
  completion_tokens = 273L,
  total_tokens = 4629L
)
```
...
Another current problem for local models, is structured output. If you use `ollama()`, it currently only supports a simpler version of structured output where you have to put the desired schema in the prompt as JSON example and enable `.json = TRUE` and it is not guaranteed that the model will adhere to a schema: 
...
```
Here we already see, that the **gemma2** model we used did not completely adhere to the required schema. In fact, we get a multirow `tibble` because the model did reply with a list for `KeyCitations` instead of a single string. 
